...
| Metric                  | Description                                                                                      |
| ----------------------- | ------------------------------------------------------------------------------------------------ |
| `wasm_memory_size`      | Heap memory used by the module’s Wasm code.                                                      |
| `stable_memory_size`    | Stable memory usage of the module.                                                               |
| `global_memory_size`    | Memory used by global variables declared in the module.                                          |
| `wasm_binary_size`      | Size of the installed Wasm binary (e.g., compressed if uploaded as gzipped).                     |
| `custom_sections_size`  | Memory used by Wasm custom sections (e.g. metadata).                                             |
| `canister_history_size` | Memory used by the module’s execution history.                                                   |
| `wasm_chunk_store_size` | Memory used for storing Wasm chunks (when using chunked installation for WASM larger than 2 MB). |
| `snapshots_size`        | Memory consumed by snapshots created for the module.                                             |
