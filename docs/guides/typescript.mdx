...
import { Assertions } from "./components/functions/assertions.mdx";

<Assertions
  language="typescript"
  code={`import { decodeDocData } from "@junobuild/functions/sdk";
import {
  defineAssert,
  type AssertSetDoc,
...
import { Call } from "./components/functions/call.mdx";

You can use `call` from `@junobuild/functions/ic-cdk` to perform inter-canister calls from your Satellite to other canisters on the Internet Computer.

<Call
  language="typescript"
  code={`import { call } from "@junobuild/functions/ic-cdk";
import {
  defineHook,
  type OnSetDoc,
} from "@junobuild/functions";
